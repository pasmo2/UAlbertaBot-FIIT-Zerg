...
void ProductionManager::manageBuildOrderQueue()
{
    ...
    if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Zerg && BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Zerg &&
        ...
        BWAPI::Broodwar->getFrameCount() / (24 * 60) < 4) {
        m_queue.removeCurrentHighestPriorityItem();
        m_queue.queueAsLowestPriority(MetaType(BWAPI::UnitTypes::Zerg_Hatchery),true);
        //printf("\nremoved hatch because of matchup being ZvZ\n");
    }
    ////if zvz, expand after 300sec
    //if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Zerg && BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Zerg &&
    //    BWAPI::Broodwar->getFrameCount() / (24*60) > 4 && UnitUtil::GetAllUnitCount(BWAPI::UnitTypes::Zerg_Hatchery) == 1) {
    //    m_queue.queueItem(BuildOrderItem(MetaType(BWAPI::UnitTypes::Zerg_Hatchery),5,false));
    //}
