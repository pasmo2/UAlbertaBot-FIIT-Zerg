...
void ScoutManager::moveScouts()
{
    ...
    //else if (enemyBaseLocation) {
    //    if (m_workerScout->getDistance(enemyBaseLocation->getPosition()) < 400) {
    //        m_workerScout->move(BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation().x * 32, BWAPI::Broodwar->self()->getStartLocation().y * 32), false);
    //        releaseScout(m_workerScout);
    //        printf("released scout SECOND CONDITION\n");
    //        return;
    //    }
    //}
    ...
    if (enemyBaseLocation)
    {
        ...
        //if (m_workerScout->getPosition().getApproxDistance(enemyBaseLocation->getPosition()) < 100) {
        //    //Micro::SmartMove(m_workerScout, BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation()));
        //    fleeScout();
        //    //m_workerm
        //    return;
        //}
            ...
            if (!m_scoutUnderAttack)
            {
               ...
               /* if (Config::Strategy::ScoutHarassEnemy && (!Config::Strategy::GasStealWithScout || m_gasStealFinished) && closestWorker && (m_workerScout->getDistance(closestWorker) < 800))
                ...
                {*/
                    ...
                    m_workerScout->move(enemyBaseLocation->getPosition(),false);
                    /*if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Zerg) {
                        printf("queueing dance, then releasin the scout");
                        m_workerScout->move(BWAPI::Position(enemyBaseLocation->getPosition().x + 250, enemyBaseLocation->getPosition().y + 250), true);
                        m_workerScout->move(BWAPI::Position(enemyBaseLocation->getPosition().x - 250, enemyBaseLocation->getPosition().y + 250), true);
                        m_workerScout->move(BWAPI::Position(enemyBaseLocation->getPosition().x - 250, enemyBaseLocation->getPosition().y - 250), true);
                        m_workerScout->move(BWAPI::Position(enemyBaseLocation->getPosition().x + 250, enemyBaseLocation->getPosition().y - 250), true);
                    }*/
                    //printf("RELEASING SCOUT, CONDITION 3\n");
                    //releaseScout(m_workerScout);
            }
...
void ScoutManager::releaseScout(BWAPI::Unit workerScoutUnit)
{
    m_workerScout = nullptr;
    Global::Workers().setMineralWorker(workerScoutUnit);
    return;
}
