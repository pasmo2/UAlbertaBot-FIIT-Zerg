...
void BaseLocationManager::onFrame()
{
    ...
    //{//changed -> perhaps not the best to spam this into the console all the time
...
BWAPI::TilePosition BaseLocationManager::getNextExpansion(BWAPI::Player player) const
{
    ...
    for (auto & base : getBaseLocations())
    {
        ...
        if (base->isOccupiedByPlayer(BWAPI::Broodwar->self()) || base->isOccupiedByPlayer(BWAPI::Broodwar->enemy())) {
